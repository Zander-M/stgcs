...
from stgcs.bfs.best_first_search import DominationChecker, SearchNode
...
class AStar_DC(DominationChecker):
...
class GlobalUpperBound_DC(DominationChecker):
...
class Sampling_DC(DominationChecker):
            ...
            for _ in range(self._num_samples_per_vertex):
                try:
                    if last_st_sample is None:
                        raise RuntimeError("Expected a non-empty HPolyhedron when sampling IPC vertex sets.")
                    st_sample = HPolyhedronSampler.uniform_sample(
                        convex_set,
                        rng,
                        last_st_sample,
                        tol=1e-6,
                        context=f"Sampling_DC[{vertex_name}]",
                    )
                except Exception:
    ...
    def check(self, n:SearchNode, stgcs:STGCS, gcs:GCS) -> bool:
        ...
        for costs in frontier:
            if all(cost <= candidate_duration for cost in costs):
                logger.debug(f"SC[{n.vertex_name}]. Dominated by duration lower bound: path={n.vertex_path}")
                return True
        ...
        for costs in frontier:
            if all(c1 <= c2 for c1, c2 in zip(costs, candidate_costs)):
                dominated = True
                logger.debug(f"SC[{n.vertex_name}]. Dominated: costs={candidate_costs}, path={n.vertex_path}")
                # logger.debug(f"SC[{n.vertex_name}]={self._debug[n.vertex_name]}. Dominated: costs={candidate_costs}, path={n.vertex_path}")
                break
...
class InexactSetContainment_DC(DominationChecker):
    ...
    def __init__(
        ...
        # for each vertex, every Cone(p) does noe contain any other Cone(q)
    ...
    def check(self, n:SearchNode, stgcs:STGCS, gcs:GCS) -> bool:    
        ...
        assert not (is_dominated and dominated != []), "Logical error in domination checking"
        for q in dominated:
        ...
        if is_dominated:
            ...
            return True
        elif self.greedy_option == InexactSetContainment_DC.GreedyOptions.KEEP_EARLIEST \
...
class ExactSetContainment_DC(DominationChecker):
    ...
    def __init__(
        ...
        # for each vertex, every Cone(p) does noe contain any other Cone(q)
        self._option = option
        self._paths_on_verts: Dict[str, ExactSetContainment_DC._Frontier] = defaultdict(self._Frontier)
        self._paths_on_edges: Dict[Tuple[str, str], ExactSetContainment_DC._Frontier] = defaultdict(self._Frontier)
    ...
    def check(self, n:SearchNode, stgcs:STGCS, gcs:GCS) -> bool:    
        if self._option == ExactSetContainment_DC.Option.EDGE_ONLY:
            return self._edge_based_check(n, stgcs, gcs)
        elif self._option == ExactSetContainment_DC.Option.VERTEX_ONLY:
            return self._vertex_based_check(n, stgcs, gcs)
        elif self._option == ExactSetContainment_DC.Option.BOTH:
    ...
    def _edge_based_check(self, n:SearchNode, stgcs:STGCS, gcs:GCS) -> bool:
        ...
        path_key = n.vertex_path_tuple
        if len(path_key) < 2:
            return False # first vertex, no domination
    ...
    def _remove_dominated_paths(self, frontier: ExactSetContainment_DC._Frontier, dominated: set[tuple]) -> None:
        if not dominated:
            return
    ...
    @property
    def name(self) -> str:
        if self._option == ExactSetContainment_DC.Option.VERTEX_ONLY:
            return "ESC"
        elif self._option == ExactSetContainment_DC.Option.EDGE_ONLY:
            return "ESC(e)"
        else:
            return "ESC(b)"
...
def exact_cone_containment(
    ...
    if is_precomputed:
        ...
        enlarged_reachable_set = ExactSetContainment_DC._reachable_set(q_hpoly, q[-1])
    ...
    return ExactSetContainment_DC._all_earliest_cone_containment(p, np.asarray(verts), vmin, vmax)
