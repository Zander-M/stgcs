...
def _apply_ecd_pairs(stgcs: STGCS, ecd_pairs: List[ECDPair]) -> STGCS:
    """Split every STGCS vertex whose space-time region overlaps one of `ecd_pairs`
    into the convex free-space pieces `slice()` produces, and reconnect the graph.

    Shared by `reserve()` and `reserve_spline()` (BLACKMAGIC/bspline-migration-plan.md
    SS4 point 5): this part is generic over `HPolyhedron`/`Interval` and never
    inspects how many halfspaces are in `mid_halfspaces` or how the bounds were
    derived, so it's identical for the parallelotope-tube and general-hull cases.

    Every piece `slice()` returns is a valid GCS vertex as-is: `reserve()`/
    `reserve_spline()` inflate the reserved obstacle by `2 * robot_radius`
    (`stgcs/pbs.py`'s convention), so the *next* robot's GCS program treats its own
    reference trajectory as a point -- any nonempty leftover piece is already safe,
    with no further footprint-fit check needed on the piece itself. (An earlier
    version of this function gated pieces through a `footprint_hpoly`
    `PontryaginDifference` feasibility check; that conflated the `2R` inter-robot
    margin with a same-sized "does the robot's own body fit" test, which is the
    wrong question under the point-mass convention -- removed.)
    """
...
def reserve(
    ...
    stgcs = _apply_ecd_pairs(stgcs, ecd_pairs)

    if debug:
        plt.ion()
        if stgcs.dimension == 1:
            fig, ax = plt.subplots()
            ax.axis("equal")
            for x in trajectory:
                xp, xq = x[:2], x[2:]
                ax.plot([xp[0], xq[0]], [xp[1], xq[1]], "-.k")
                draw_parallelotope_2d(ax[1], xp, xq, 0.8 * safe_radius)
            draw_STGCS(stgcs, ax[1], edges=True)
        elif stgcs.dimension == 2:
            fig = plt.figure()
            ax = fig.add_subplot(111, projection='3d')
            ax.axis("equal")
            ax.axis("off")
            draw_STGCS(stgcs, ax, edges=True)
            fig, ax = plt.subplots()
            draw_STGCS_planar_graph(stgcs, ax)
...
# ---------------------------------------------------------------------------
# order > 2 (Bezier/B-spline-per-region) reservation path.
#
# Per BLACKMAGIC/bspline-migration-plan.md SS4: a GCS vertex already corresponds to
# exactly one Bezier segment in this architecture (SS0), so there is no separate
# "split policy" step here -- one convex hull per segment is already what today's
# per-vertex ECD does for linear segments. Only the obstacle geometry differs
# (Minkowski-inflated control-point hull instead of a direction-aligned
# parallelotope tube, via `stgcs.hulls.inflate_hull`); `slice`/`update_edge`
# (through `_apply_ecd_pairs`) are exactly the same code the order=2 path uses.
# ---------------------------------------------------------------------------
...
def _segment_ecd_pair(
    ...
    `mid_halfspaces` are the inflated hull's own (reduced) H-representation rows,
    negated to the "outside this facet" convention `slice()` expects -- exactly what
    `parallelotope_side_halfspace_kd` already builds directly for the linear-segment
    tube, just from a general hull's actual facets instead of a fixed `2*space_dim`
    axis-aligned set.

    Bounds come from the raw Minkowski-sum point cloud (`minkowski_sum_vertices`),
    not from re-deriving the hull's vertices via `VPolytope`: a set's per-axis min/max
    is achieved at an extreme point, so any non-extreme points already present in the
    raw sum can't change it. Computing bounds this way instead of a second H-to-V
    conversion skips a qhull round-trip per segment that would otherwise duplicate the
    ConvexHull already run for `inflated_hpoly` below.
    """
...
def generate_all_ECD_pairs_spline(
    ...
    """order > 2 counterpart to `generate_all_ECD_pairs`: one Minkowski-inflated hull
    per GCS-vertex segment, instead of a parallelotope tube per linear segment.
    """
    dim = stgcs.dimension + 1
    ret: List[ECDPair] = []
...
def reserve_spline(
    ...
    """order > 2 counterpart to `reserve()`. `safe_radius` gives the same isotropic
    per-axis margin semantics as the linear path, built into an axis-aligned box
    footprint via `parallelotope_verts_offset` (zero time-extent, matching AGENT.md:
    "a footprint doesn't occupy a time interval, only a spatial one").
    """
    tmin = stgcs.t0 if x0_staying else None
    tmax = stgcs.tmax if xt_staying else None
    cache_key = None
...
# swept hulls happen to overlap (AABB-gated). That is the right behavior for "a robot's
# full reservation", but it presupposes a whole trajectory. The three functions below
# are the atomic step underneath it, exposed directly: given exactly one already-known
# GCS vertex (one region traversal) and exactly one Bezier segment's control points
# (design note SS4 "Fix A" -- a GCS vertex already *is* one Bezier segment, F1-F3), build
# that segment's decision boundaries and slice only that named region against them.
# Useful on its own wherever the caller already knows which region a segment lives in
# (e.g. building a reservation up region-by-region, or a focused demo) without paying
# for a graph-wide overlap scan.
# ---------------------------------------------------------------------------


def bezier_segment_hull(control_points: np.ndarray, footprint_vertices: np.ndarray) -> HPolyhedron:
    """Step 1: the segment's Minkowski-inflated space-time control-point hull (F1, F7).

    Thin, named wrapper around `hulls.inflate_hull` -- kept as its own step so the
    pipeline below reads as the same stages AGENT.md SS3's reservation pipeline lists
    (hull -> decision boundaries -> subtract), rather than one opaque call.
    """
...
def bezier_decision_boundaries(inflated_hull: HPolyhedron) -> List[HPolyhedron]:
    """Step 2: the inflated hull's own facets, as "outside this facet" halfspaces.

    Each row of the hull's (reduced) H-representation `A @ x <= b` is a supporting
    hyperplane of the obstacle; negating it flips "inside the obstacle" to "outside
    it", which is the convention `slice()` peels pieces against (reservation-geometry-
    design-note.md SS3: "every decision boundary ECD introduces can touch obstacle
    boundaries without cutting any obstacle interior"). Identical construction to what
    `_segment_ecd_pair` inlines for `mid_halfspaces`; separated out here so a caller
    (or the demo notebook) can inspect/plot the boundaries themselves before slicing.
    """
...
def reserve_region_bezier(
    ...
    Not part of the production reservation path -- nothing in `stgcs` calls this
    outside of its own unit test (`tests/unit/test_reserve_region_bezier_unittest.py`)
    and `notebooks/ecd_bezier_visualization.ipynb`. Real callers want `reserve_spline`,
    which does the whole-trajectory bookkeeping this function deliberately omits (see
    below). This exists as the single-region, single-segment atomic primitive for
    testing/demonstration: it lets a caller inspect or verify one region's slicing in
    isolation, and it's what the notebook builds up from first principles. Composing
    calls to this function to reproduce `reserve_spline`'s whole-trajectory result
    exactly is possible but non-trivial -- see the notebook's Part 7 for the two rules
    that make it work (check every region's original extent for an AABB touch, not
    just a segment's home region; reserve against each touched region's current
    surviving pieces via `root_name`, not a stale vertex name).

    Step 3+4: build the segment's `ECDPair` (hull + decision boundaries + time crop,
    via `_segment_ecd_pair` -- steps 1-2 above inlined, the same construction
    `generate_all_ECD_pairs_spline` uses per segment) and slice *only* `region_name`
    against it, reconnecting that region's neighbors to whichever leftover pieces
    survive. This is `reserve_spline`'s per-segment inner step, minus the loop over
    `trajectory.size` segments and minus `_apply_ecd_pairs`'s graph-wide AABB scan --
    both replaced here by the caller naming the one region directly.

    A segment whose swept, inflated hull never actually reaches into `region_name`
    (e.g. a segment queried against the wrong region by mistake) is a no-op: the AABB
    check below is the same gate `_apply_ecd_pairs` uses, just evaluated for one vertex
    instead of scanning the whole graph for it.
    """
    dim = stgcs.dimension + 1
