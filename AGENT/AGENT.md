...
You are implementing a research prototype in multi-robot motion planning. Read this whole file before writing code. F1-F9 below are stated in full inline (§2) — there is no separate derivation note in *this* repo; `reservation-geometry-design-note.md` and `bspline-migration-plan.md`, referenced by file-relative name elsewhere in this repo's `AGENT/` docs (e.g. `AGENT/ecd.py`, `porting-bezier-to-search-based-stgcs.md`), live in the sibling relaxation+rounding repo (`stgcs-devel`) this design originated from, not here — do not go looking for them in this tree. For *this* repo's concrete, file-by-file integration plan against the actual `stgcs/` codebase (grounded in real file:line references, with implementation status), see `search-based-bezier-implementation-plan.md`; some of its later phases, e.g. `pbs.py`/`windowed_coordination.py` wiring, are outside this phase's scope — see below.
...
**F3 — Segments own their control points by construction, not by extraction.** Because each GCS vertex's control points are private decision variables, consecutive segments already share only a C⁰ join point ⇒ disjoint relative interiors ⇒ separable, with no knot-insertion step required to get there. C¹ (and optionally C²) continuity across the join is imposed instead as linear equality constraints between the neighboring vertices' boundary control points on the GCS edge — invisible to the decomposition, the same effect the extraction-based scheme gets from "continuity migrates into linear relations between neighbors' control points," just derived directly instead of via extraction. This is the concrete mechanism for "continuity between Bézier curves" along one robot's path. C² consumes one more control point per side than C¹ (matching up to Cᶜ leaves ~order−(c+1) free control points per segment); empirically this makes order=3 (quadratic) infeasible against anything but a trivially loose region (0 free points) and order≥4 (cubic and up) robust (1+ free points, verified down to a 0.1-wide bent corridor and a 3-bend zigzag chain) — implemented gated to `order >= 4` in `stgcs/graph.py`.
...
**F9 — Uniform-time opt-in (enables a sound acceleration certificate; the bound itself is future work).** F8 is exact because dP/dt = P′(s)/T′(s) is a plain quotient (inverse-function rule, no product term) and T′(s) > 0 always (F4's monotonicity). d²P/dt² is not: the quotient rule introduces a cross term, d²P/dt² = [P″(s)T′(s) − P′(s)T″(s)] / T′(s)³, and T″(s) is not sign-definite under mere monotonicity — the time reparametrization is free to speed up or slow down in s. An F8-style per-control-point SOC bound applied naively to the raw second differences would therefore silently drop that cross term and certify nothing about the true physical acceleration. Forcing a segment's time control points into an arithmetic sequence (`uniform_time=True` in `stgcs/graph.py`) makes T(s) exactly affine in s — T″(s) ≡ 0 identically, not approximately (a Bézier curve with equally-spaced control points reduces exactly to the line through its endpoints) — and T′(s) becomes the positive *constant* T_{order-1}−T₀. The cross term vanishes, d²P/dt² collapses to P″(s)/T′(s)², and F8's exact triangle-inequality/convex-hull argument applies unchanged one derivative up. This is the same simplification Drake's `KinematicTrajectoryOptimization` and the GCS trajectory-optimization literature (manipulation.csail.mit.edu/trajectories.html) build in from the start — they never give time its own per-control-point Bézier; duration is a single scalar (or a `b(s) = ṡ²` time-scaling) per segment, decoupled from the spatial path. Trade-off: interior time allocation, previously free under F4's monotonicity-only constraint, is pinned to equal spacing whenever this mode is on — a strictly smaller feasible region, verified non-empty (order 3-6, bent-corridor scenarios) but with generally worse (never better) optimal duration than free allocation. `uniform_time` fixes only the timing side of the certificate; the acceleration SOC bound analogous to F8 is not yet implemented.
...
**This is a conceptual grouping of concerns, not the layout actually used by any real integration.** In *this* repo, per the resolved decision in `search-based-bezier-implementation-plan.md` (top of file), the code stays inside the existing `stgcs/` package rather than standing up a parallel `src/` tree: `bezier/` → `stgcs/bezier.py`, `hulls/` → `stgcs/hulls.py`, `decomposition/` → `stgcs/ecd.py`, `gcs/` → `stgcs/graph.py` + `stgcs/gcs_solver.py` + `stgcs/spline_trajectory.py`, `planner/` → `stgcs/st_planner.py`/`stgcs/bfs/`. There is still no `tests/` directory (see that doc's Step 8).

## 5. Implementation Order

(Module names below are §4's conceptual grouping; see §4's note for where each one actually lives in `stgcs/`.)
