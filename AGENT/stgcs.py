...
from stgcs.visualization_utils import draw_2d_set, draw_3d_set
...
class STGCS:
    ...
    def __init__(
        ...
        # order = control points per GCS vertex. 2 = today's linear segment (default,
        # unchanged behavior); 4 = cubic Bezier segment per vertex (BLACKMAGIC/AGENT.md).
        self.order = order
    ...
    def __getstate__(self) -> Dict[str, object]:
        ...
        state["edge_constraints_boundary"] = []
    ...
    def _add_gcs_edge_costs_constraints(
        ...
        # Edges touching a source/target/dummy-target vertex get C0-only continuity: those
        # vertices have no well-defined internal velocity (see BLACKMAGIC/AGENT.md, "boundary
        # edges"), so requiring the C1 tangent-match there would spuriously force a zero
        # velocity at the start/end of the path. Edges between two real per-region vertices
        # get the full C0+C1 constraint list (order > 2 only; identical to edge_constraints
        # for order == 2, where only C0 ever existed).
    ...
    def _init_constraints_costs(self) -> None:
        ...
        # Joint control points Q_0..Q_{order-1} = (P_i, T_i); flat layout per vertex is
        # [P_0, T_0, P_1, T_1, ..., P_{order-1}, T_{order-1}], each block of size `block`.
        n = order * block
        ...
        # F8 (time side): each consecutive pair needs >= dt (implies monotonicity, and
        # is strictly stronger than an overall T_{order-1}-T_0 >= dt). A per-span floor,
        # rather than only an overall one, rules out the degenerate solution where the
        # solver collapses an interior control-point spacing to ~0 (legal under only an
        # overall bound, but leaves that span's velocity ill-conditioned) -- interior
        # time *allocation* is still free (uncosted), just bounded away from zero.
        ...
        # cost is total segment duration only (T_{order-1} - T_0), not interior spans
        ...
        # F8 (velocity side): per-span SOC constraint ||P_{i+1}-P_i|| <= vlimit*(T_{i+1}-T_i),
        # replacing the L-infinity box used for order == 2.
        for i in range(order - 1):
        ...
        # F3 (edge continuity): C0 always (tail's last control point == head's first);
        # C1 additionally, only between two real per-region vertices (see
...
def draw_STGCS(stgcs:STGCS, ax:Axes|Axes3D, edges=False, set_labels=True, bounds:List[Interval]=[]) -> None:
    if isinstance(ax, Axes3D):
        assert stgcs.dimension == 2, "3D plotting is only supported for 3D STGCS"
        c = ['r', 'c', 'b', 'y', 'g', 'm']
        for i, v_name in enumerate(stgcs.G.nodes):
            v = stgcs.get_vertex(v_name)
            if v_name[0] != 'v' or (bounds != [] and not AABB(v.space_itvls + [v.time_itvl], bounds)):
                continue
            center = np.mean(VPolytope(v.st_hpoly).vertices(), axis=1)
            draw_3d_set(v.st_hpoly, ax, fc=c[i%len(c)], alpha=0.2)
            if set_labels:
                ax.text(center[0], center[1], center[2], v_name, color='k')
        
        if edges:
            for u_name, v_name in stgcs.G.edges:
                if u_name != 'v' or v_name != 'v':
                    continue
                u, v = stgcs.get_vertex(u_name), stgcs.get_vertex(v_name)
                P = VPolytope(u.st_hpoly).vertices()
                Q = VPolytope(v.st_hpoly).vertices()
        ...
        assert stgcs.dimension == 1, "2D plotting is only supported for 2D STGCS"
        c = ['r', 'c', 'b', 'y', 'g', 'm', 'k']
        for i, v_name in enumerate(stgcs.G.nodes):
            v = stgcs.get_vertex(v_name)
            if v_name != 'v':
                continue
            center = np.mean(VPolytope(v.st_hpoly).vertices(), axis=1)
            draw_2d_set(v.st_hpoly, ax, color=c[i%len(c)])
            if set_labels:
                ax.text(center[0], center[1], v_name, color='k')
...
def draw_STGCS_with_solution(stgcs:STGCS, ax:Axes3D, sol:STTrajectory, set_labels=True, bounds:List[Interval]=[]) -> None:
    c = ['r', 'c', 'b', 'y', 'g', 'm', 'k']
    for i, (v_name, wp) in enumerate(zip(sol.vertex_path, sol.points)):
        v = stgcs.get_vertex(v_name)
        xp, xq = wp[:3], wp[3:]
        if v_name[0] != 'v' or (bounds != [] and not AABB(v.space_itvls + [v.time_itvl], bounds)):
            continue
        ...
        center = np.mean(VPolytope(v.st_hpoly).vertices(), axis=1)
        draw_3d_set(v.st_hpoly, ax, fc=c[i%len(c)], alpha=0.25)
        if set_labels:
            ax.text(center[0], center[1], center[2], v_name, color='k')


def draw_STGCS_planar_graph(stgcs:STGCS, ax:Axes, with_labels=True) -> None:
    ax.axis("equal")
    pos = nx.layout.spring_layout(stgcs.G)
    nx.draw(stgcs.G, pos, ax=ax, with_labels=with_labels)
    # highlight source and target
    if stgcs.source is not None:
        nx.draw_networkx_nodes(stgcs.G, pos, nodelist=[GCS_SOURCE_NAME], node_color='r', ax=ax)
    if stgcs.target is not None:
        nx.draw_networkx_nodes(stgcs.G, pos, nodelist=[GCS_TARGET_NAME], node_color='g', ax=ax)
