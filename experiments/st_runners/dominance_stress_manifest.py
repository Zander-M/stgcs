...
class STDominanceStressCandidate:
    record: STHeuristicAblationRecord


class STDominanceStressManifestBuilder:
    ...
    @staticmethod
    def edge_gap(candidate: STDominanceStressCandidate, target_edges: int) -> tuple[int, int, str]:
                ...
                for obstacle_count in obstacle_counts:
                    for attempt in range(int(attempts_per_count)):
                        ...
                        if not (accepted_min_edges <= int(record.stgcs_num_edges) <= accepted_max_edges):
                            continue
                        candidates.append(STDominanceStressCandidate(record=record))
        return sorted(
            candidates,
    ...
    def build_candidate_with_cache(
        ...
        candidate_cache: dict[tuple[str, str, int, int], STDominanceStressCandidate | None],
    ) -> STDominanceStressCandidate | None:
        ...
        candidate = None if record is None else STDominanceStressCandidate(record=record)
    ...
    def find_candidate_for_target(
        ...
        best: STDominanceStressCandidate | None = None
    ...
    def select_records_for_targets(
        ...
        candidates: Sequence[STDominanceStressCandidate],
...
class STDominanceStressManifestCLI:
    @classmethod
    def parse_args(cls) -> argparse.Namespace:
        parser = argparse.ArgumentParser(
            description=(
                "Build an ST dominance-check stress-test manifest from existing data/stgcs_base records "
                "by adding dynamic obstacles until the constructed ST-GCS is near the target size."
            )
        )
        parser.add_argument("--base-root", type=Path, default=STDominanceStressManifestBuilder.DEFAULT_BASE_ROOT)
        parser.add_argument("--output", type=Path, default=STDominanceStressManifestBuilder.DEFAULT_OUTPUT)
        parser.add_argument(
            "--count",
            "--k",
            dest="count",
            type=int,
            default=STDominanceStressManifestBuilder.DEFAULT_COUNT,
        )
        parser.add_argument("--min-edges", type=int, default=STDominanceStressManifestBuilder.DEFAULT_MIN_EDGES)
        parser.add_argument("--max-edges", type=int, default=STDominanceStressManifestBuilder.DEFAULT_MAX_EDGES)
        parser.add_argument(
            "--edge-tolerance",
            type=float,
            default=STDominanceStressManifestBuilder.DEFAULT_EDGE_TOLERANCE,
        )
        parser.add_argument(
            "--obstacle-counts",
            type=int,
            nargs="+",
            default=STDominanceStressManifestBuilder.DEFAULT_OBSTACLE_COUNTS,
        )
        parser.add_argument(
            "--attempts-per-count",
            type=int,
            default=STDominanceStressManifestBuilder.DEFAULT_ATTEMPTS_PER_COUNT,
        )
        parser.add_argument(
            "--base-candidate-limit",
            type=int,
            default=STDominanceStressManifestBuilder.DEFAULT_BASE_CANDIDATE_LIMIT,
        )
        parser.add_argument(
            "--append-existing",
            action="store_true",
    ...
    def main(cls) -> None:
        ...
        if output_path.exists():
            ...
            STDominanceStressManifestBuilder.validated_existing_counts_by_domain(
                existing_records,
                count,
                STDominanceStressManifestBuilder.DEFAULT_DOMAIN_KEYS,
            )
        ...
        if existing_records is not None and (args.append_existing or len(existing_records) < count):
            ...
            records = STDominanceStressManifestBuilder.extend_manifest(
            ...
            print(
                f"Appended {len(records) - original_count} ST dominance stress-test records "
                f"to {output_path.resolve()}"
            )
        elif existing_records is not None:
            records = list(existing_records)
            print(
                f"Existing ST dominance stress-test manifest already has "
            ...
            print(
                f"Appended {len(records) - len(existing_records)} ST dominance stress-test records "
                f"to {output_path.resolve()}"
            )
        else:
            records = STDominanceStressManifestBuilder.build_manifest(
                base_root=Path(args.base_root),
                count=count,
                min_edges=int(args.min_edges),
                max_edges=int(args.max_edges),
            ...
            print(f"Wrote {len(records)} ST dominance stress-test records to {output_path.resolve()}")


if __name__ == "__main__":
    STDominanceStressManifestCLI.main()
