...
MANIFEST="${MANIFEST:-data/instances/st_planning/stress_test/manifest.json}"
BASE_ROOT="${BASE_ROOT:-data/stgcs_base}"
OUTPUT_ROOT="${OUTPUT_ROOT:-data/results/st_planning/performance_comparison}"
BUDGET="${BUDGET:-600}"
...
for instance_id in ${instance_ids[@]+"${instance_ids[@]}"}; do
    for planner in ${planner_keys[@]+"${planner_keys[@]}"}; do
        ...
        if run_with_timeout "$TIMEOUT_SECS" \
            "$PYTHON_BIN" -m experiments.st_runners.performance_comparison_micp_wrapper \
            "$MANIFEST" \
            --base-root "$BASE_ROOT" \
