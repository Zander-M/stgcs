...
class STDominanceAblationGroupReport(STHeuristicAblationGroupReport):
    DEFAULT_MANIFEST = Path("data/instances/st_planning/manifest.json")
    DEFAULT_RESULTS_ROOT = Path("data/results/st_planning/dominance_ablation")
    DEFAULT_OUTPUT_PREFIX = Path("latex/figs/dc_ablation")

    PLANNERS = (
        "Search(h_max+GUB)",
        "Search(h_max+GUB+delta_set)",
        "Search(h_max+GUB+delta_state)",
        "Search(h_max+GUB+delta_pos)",
    )
    ...
    PLANNER_SHORT_LABELS = {
        "Search(h_max+GUB)": "UB",
        "Search(h_max+GUB+delta_set)": r"$+\delta_\text{set}$",
        "Search(h_max+GUB+delta_pos)": r"$+\delta_\text{pos}$",
        "Search(h_max+GUB+delta_state)": r"$+\delta_\text{state}$",
    }
    PAIRS = (
        ("Search(h_max+GUB+delta_set)", "Search(h_max+GUB+delta_pos)"),
        ("Search(h_max+GUB+delta_set)", "Search(h_max+GUB+delta_state)"),
        ("Search(h_max+GUB+delta_pos)", "Search(h_max+GUB+delta_state)"),
    )
    METRIC_FIELDS = {
    ...
    EXPANDED_NODE_GAP_Y_LABEL_COORDS = {
        "Search(h_max+GUB+delta_pos)": (-0.15, 0.5),
        "Search(h_max+GUB+delta_state)": (-0.15, 0.5),
    }
    ...
    FIGURE_HEURISTIC_COLORS = {
        "Search(h_max+GUB)": STHeuristicAblationGroupReport.FIGURE_HEURISTIC_COLORS[
            "Search(h_mot+GUB)"
        ],
        "Search(h_max+GUB+delta_set)": STHeuristicAblationGroupReport.FIGURE_HEURISTIC_COLORS[
            "Search(h_tri+GUB)"
        ],
        "Search(h_max+GUB+delta_pos)": STHeuristicAblationGroupReport.FIGURE_HEURISTIC_COLORS[
            "Search(h_max+GUB)"
        ],
        "Search(h_max+GUB+delta_state)": STHeuristicAblationGroupReport.FIGURE_HEURISTIC_COLORS[
            "Search(h_tab+GUB)"
        ],
    }
    FIGURE_Y_TICKS = {
    ...
    FIGURE_SIZE = (6, 5)
    REFERENCE_PLANNER = "Search(h_max+GUB+delta_set)"
    GAP_PLANNERS = (
        "Search(h_max+GUB+delta_state)",
        "Search(h_max+GUB+delta_pos)",
    )
...
class STDominanceAblationGroupReportCLI:
    @classmethod
    def parse_args(cls) -> argparse.Namespace:
        ...
        parser.add_argument(
            "--results-root",
            type=Path,
            default=STDominanceAblationGroupReport.DEFAULT_RESULTS_ROOT,
        )
        parser.add_argument(
            "--manifest",
            type=Path,
            default=STDominanceAblationGroupReport.DEFAULT_MANIFEST,
            help="Filter plotted rows to instance IDs in this manifest.",
        )
        parser.add_argument(
            "--output-prefix",
            type=Path,
            default=STDominanceAblationGroupReport.DEFAULT_OUTPUT_PREFIX,
        )
    ...
    def main(cls) -> None:
        args = cls.parse_args()
        rows_by_planner = STDominanceAblationGroupReport.load_rows_by_planner(
            args.results_root,
            args.manifest,
        )
        budget = STDominanceAblationGroupReport.select_budget(rows_by_planner, args.budget)
        STDominanceAblationGroupReport.plot_boxplot_figure(
            rows_by_planner,
            budget,
            "quality",
            args.output_prefix,
...
if __name__ == "__main__":
    STDominanceAblationGroupReportCLI.main()
